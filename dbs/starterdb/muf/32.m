...
: main ( s -- )
  ...
  begin depth 1 > while  ( ix..i2 i1 )
    over descridle  ( ix..i2 i1 it2 }  it2=number of seconds i2 has been connected )
    over descridle  ( ix..i2 i1 it2 it1 )
    ( If it2 is greater than it1, we'll want to boot i2, not i1. )
    > if  ( ix..i2 i1 )
      ( Get the one we should boot on top of the stack. )
      swap  ( ix..i1 i2 )
    then  ( ix..i? i? )
    dup "You have been booted by yourself." descrnotify descrboot  ( ix..i? )
  repeat  ( i }  i=the one descriptor left over )
  "Old connections booted." descrnotify  (  )
;
