...
struct tune_str_entry tune_str_list[] = {
	{"Commands",	"autolook_cmd", &tp_autolook_cmd, 0, MLEV_WIZARD, "Room entry look command", "", 0, 1, AUTOLOOK_CMD},
	{"Currency",	"cpennies", &tp_cpennies, 0, MLEV_WIZARD, "Currency name, capitalized, plural", "", 0, 1, CPENNIES},
...
struct tune_time_entry tune_time_list[] = {
	{"Database",	"aging_time", &tp_aging_time, 0, MLEV_WIZARD, "When to considered an object old and unused", "", 1, AGING_TIME},
	{"DB Dumps",	"dump_interval", &tp_dump_interval, 0, MLEV_WIZARD, "Interval between dumps", "", 1, DUMP_INTERVAL},
...
struct tune_bool_entry tune_bool_list[] = {
	...
	{"Misc",	"ignore_support", &tp_ignore_support, MLEV_MASTER, MLEV_WIZARD, "Enable support for @ignoring players", "", 1, IGNORE_SUPPORT},
	{"Misc",	"ignore_bidirectional", &tp_ignore_bidirectional, MLEV_MASTER, MLEV_WIZARD, "Enable bidirectional @ignore", "", 1, IGNORE_BIDIRECTIONAL},
	...
	{"WHO",		"secure_who", &tp_secure_who, 0, MLEV_WIZARD, "Disallow WHO command from login screen and programs", "", 1, SECURE_WHO},
	{"WHO",		"use_hostnames", &tp_hostnames, 0, MLEV_WIZARD, "Resolve IP addresses into hostnames", "", 1, HOSTNAMES},
	{"WHO",		"who_doing", &tp_who_doing, 0, MLEV_WIZARD, "Show '_/do' property value in WHO lists", "", 1, WHO_DOING},
...
tune_save_parms_to_file(FILE * f)
{
	...
	while (tbool->name) {
		if (tbool->isdefault) {
			fprintf(f, "%c%s=%s\n", TP_FLAG_DEFAULT, tbool->name, (*tbool->boolval) ? "yes" : "no");
		} else {
