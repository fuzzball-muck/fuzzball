 ...
 *       server lags out, but the current timeouts on the SMTP send as
 *       *hardcoded* (unfortunately) in the library are utterly enormous.
 *
 *       This will allow us to control the timeouts.  In the future, I
 *       think we'll need to fork the smtp sends into another process.
 *       However, as a first stab this isn't too bad.
 *
 *       Note that this timeout applies to every read operation so technically
 *       it is additive -- for a 20 second timeout, if one read takes 19
 *       seconds, then the next takes 19 seconds, and so forth, you could be
 *       waiting a long time.  Thus, make this pretty small.
 */
#define SMTP_READ_TIMEOUT_SEC 50
